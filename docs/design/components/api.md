...
| `POST` | `/api/games/{id}/decisions` | решения по карточкам: `accept` — совет, `accept:N` — вариант N, `reject` |
...
а у прожитого — отчёт и журнал. У карточки — варианты правки с прогоном линтера и
совет. Сервер берёт советника-модель флагами `-mode`, `-model`, `-records`. Типы вида строятся в `api`; домен их не знает.
