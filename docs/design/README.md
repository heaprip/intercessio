...
Стопку и советника, выбирающего правку из меню, держит `agenda` — гипотеза помощника;
как auctor пишет норму сам, не решено — см. «Не решено» ниже.
...
| дело, пересмотр, intercessio, вступление в силу, исполнение, проверка обязанностей | `case` |
| знание норм | `actors` |
| фаза | `turn` |
...
| --- | --- | --- |
| правка, которую пишет сам auctor | `front` с редактором структуры; советник, переводящий прозу | стопку и советника держит `agenda` (гипотезы P39, P70); писать норму самому нечем |
...
`scenario` требует типов `corpus` и `facts`. К вехе `playable-case` решены ещё три
пункта: объявление должности — данные нормы в `corpus`, проверка обязанностей и
выборка — внутри `case`, состав записи — у `journal`.
...
- **Правила общие, а не сюжетные.** «Усыновлённый получает статус усыновителя»,
  а не «плебейство от плебея»; повинность берётся из пучка обязанностей статуса,
  а не пишется для гражданина. Сюжетное правило отвечает на свой казус и ломает
  соседний — прототип вычислителя показал это на усыновителе-патриции. Решение
  автора.
- **Отрицательное условие требует умолчания.** Отрицания по неудаче нет:
  `¬held(P, Prev)` в теле выполнится, только если его выводит правило по
  умолчанию, вытесняемое фактом.
