...
> **Больше не рекомендуется, замена не принята.** Решение принималось под
> прежнюю рамку — внешних интеграторов. У игры один потребитель, собственный
> фронт, и поток отчётов периода вместо ресурсно-ориентированного REST. Текст
> сохранён как есть; выбор транспорта делается заново на вехе [[docs/design/stages/external-view|external-view]] вместе с первым
> реальным контрактом.
